...
- [ ] TO VERIFY ON PROD (registered-only, not locally exercisable): current-password reauth actually rejects a wrong password; delete dialog + flow end to end on the demo. No migration needed for these (pure client/auth logic).
- [ ] STILL OPEN in Phase 3 (larger, separate surface — HouseholdView): leave-household / remove-member consequence dialogs, seat limit + open invitations + paid extra seats. Broker/provider switch unsaved-change guard (tab switch already preserves state in one component, so no silent discard there). Deferred pending owner go-ahead.
