...
seeded unlimited except `householdMembers`). Pure resolution and the grandfathering rule live in
...
### Extra seats are bought, not unlocked (migrations 0125 + 0128)

The cap is the same on both plans (`householdMembers` = 2 / 2): Pro buys the
sharing, not the head count. A third person is a Stripe subscription ITEM on
the household owner's existing subscription (`billing_config
.household_member_price`), added by `POST /api/billing/household-seat` and
mirrored into `household_seat_addons` by the same route and by the webhook, so
a seat cancelled in Stripe's portal drops the quantity here too.

Three places must therefore agree on "base + active seats": the household view
(`memberLimit`), `enforce_household_member_limit()`, and `plan_limits`. The
trigger reads its base from `plan_limits` for exactly that reason — and each
`select ... into` needs its own variable, because a household with no add-on
row matches nothing and a NULL cap silently enforces nothing at all (the 0125
bug, fixed in 0128).
