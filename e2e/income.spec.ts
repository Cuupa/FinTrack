...
// recurring entry. The mask and ledger live on the Bookings tab of /accounts,
// the recurring card on the Recurring tab (spec §10).
...
test("income can be promoted to a recurring entry, not just expenses", async ({ page }) => {
  ...
  // The promoted row stops offering the action a second time (still on the
  // bookings ledger).
  await expect(ledger.filter({ hasText: "Salary" }).getByRole("button", action)).toHaveCount(0);

  // It lands in the merged recurring card on the Recurring tab. "Goes to"
  // answers the question the overview could not: this one is credited, it does
  // not move to another account and is not consumed.
